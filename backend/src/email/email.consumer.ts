...
export class EmailConsumer {
  ...
  async handleSendDetectOtherDevice(
    @Payload() data: { to: string; userAgent: string; userIp: string },
  ) {
  ...
  async handleNotificaitonChangePassword(
    @Payload()
    data: {
      to: string;
      userName: string;
    ...
    await this.emailService.sendNotificationChangePassword(
      data.to,
      data.userName,
      data.userIp,
      data.userAgent,
  ...
  async handleSendVerifyShop(
    @Payload() data: { to: string; linkVerify: string },
  ) {
