...
export class CheckoutAndCart {
  ...
  private async storeProductToCart(
    userId: string,
    productId: string,
    quantity: number,
  ) {
    ...
    const availableProduct = await this.prismaService.sku.findUnique({
      where: { id: productId },
    });
  ...
  async handleCheckout(
    ...
    switch (checkout_prompt) {
      case 'add_product_to_cart': {
        if (!userId || !productId || !quantity) {
          throw new BadRequestException(
            'userId, productId và quantity là bắt buộc để thêm vào giỏ hàng',
          );
        }
        const message = this.checkOutAndCart.get('add_product_to_cart') || '';
        const product = await this.storeProductToCart(
          userId,
          productId,
          quantity,
        );
      ...
      case 'update_quantity_product': {
        const message =
          this.checkOutAndCart.get('update_quantity_product') || '';
        const result = await this.cartService.updateQuantityProductInCart(
          userId,
          cartId,
          storeProductId,
          quantity,
