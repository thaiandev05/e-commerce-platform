...
export class GenemiChatBotService {
	...
	constructor(
		...
		try {
			...
			const genemiVersion = configService.getOrThrow<string>("GENEMI_MODEL") || "gemini-1.5-flash"
	...
	private async firstResponse(userId: string) {
		...
		const availableUser = await this.prismaService.user.findUnique({ where: { id: userId } })
