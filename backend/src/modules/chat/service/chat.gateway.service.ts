...
export class ChatgateWayService implements OnModuleDestroy {
	...
	async joinConversation(socketId: string, roomId: string, userId: string) {
		try {
			...
			const hasPermission = room.supportId === userId || room.clientId === userId;
			if (!hasPermission) {
				this.logger.warn(`User ${userId} does not have permission to join room ${roomId}`);
				return {
					success: false,
			...
			pipeline.hset(roomUsersKey, userId, JSON.stringify({
				socketId,
				joinedAt: new Date().toISOString(),
				username: room.supportId === userId ? room.support.username : room.client.username,
				fullname: room.supportId === userId ? room.support.fullname : room.client.fullname,
			...
			this.logger.log(`User ${userId} successfully joined room ${roomId} with socket ${socketId}`);
	...
	async handleUserDisconnect(socketId: string, userId: string) {
		try {
			...
			pipeline.hset('user_status', userId, JSON.stringify({
				status: 'offline',
			...
			this.logger.log(`Cleaned up disconnect for user ${userId}, socket ${socketId}`);
		} catch (error) {
	...
	async sendMessage(data: {
		...
		try {
			...
			const hasAccess = room.supportId === data.senderId || room.clientId === data.senderId;
			if (!hasAccess) {
				return {
					success: false,
					error: 'Access denied'
