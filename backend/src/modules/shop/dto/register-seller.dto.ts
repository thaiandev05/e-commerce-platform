import { IsString, IsOptional, IsArray, IsEmail, IsPhoneNumber, IsUrl, IsEnum, IsNotEmpty, Length, Matches, ValidateNested } from 'class-validator'
...
export class RegisterSellerDto {
	...
	@IsArray()
	@IsString({ each: true })
	@Matches(/^\d{13,19}$/, { each: true, message: 'Invalid credit card number format' })
	...
	@ApiPropertyOptional({ description: 'VAT registration number (for EU sellers)' })
	@IsOptional()
	@IsString()
	vatNumber?: string
