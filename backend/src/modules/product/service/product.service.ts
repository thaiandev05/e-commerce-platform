...
export class ProductService {
  ...
  private async getUserById(userId: string) {
    if (userId === 'unknow')
      throw new BadRequestException('Userid is not received');
  ...
  public async getSkubyId(productId: string) {
    return await this.prismaService.sku.findUnique({
      where: { id: productId },
    });
  }
  ...
  async createSpu(req: Request, dto: CreateSpuDto) {
    ...
    const createSpuData = {
      ...dto,
      status: dto.status
        ? SpuStatus[dto.status as keyof typeof SpuStatus]
        : undefined,
    };
  ...
  async orderProduct(req: Request, dto: OrderProductDto) {
    ...
    const basePrice =
      Number(availableSkus?.salePrice || 0) * dto.quantityProduct;
    ...
    const newOrderProduct = await this.prismaService.$transaction([
      ...
      this.prismaService.shop.update({
        where: { id: dto.shopId },
        data: {
          quantityProduct: BigInt(
            Number(availableShop?.quantityProduct) - dto.quantityProduct,
          ),
        },
      }),
    ]);
  ...
  async cancelOrder(req: Request, orderProductId: string) {
    ...
    const availableOrderProduct =
      await this.prismaService.orderProduct.findUnique({
        where: { id: orderProductId },
      });
    if (!availableOrderProduct)
    ...
    // check permission
    if (availableUser.id !== availableOrder.ownId)
    ...
    await this.prismaService.$transaction([
      ...
      this.prismaService.shop.update({
        ...
        data: {
          quantityProduct:
            Number(availableShop.quantityProduct) + availableOrder.quantity,
        },
      }),
  ...
  async getOrder(userId: string, orderId: string) {
    ...
    const avaiableOrder = await this.prismaService.order.findUnique({
      where: { id: orderId },
    });
