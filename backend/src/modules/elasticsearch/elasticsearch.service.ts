...
export class ElasticsearchServiceCustom {
  ...
  async handleProductCreated(payload: { req: any; newSpu: Spu }) {
    try {
      ...
      this.logger.error(
        `Failed to index new product ${payload.newSpu.id}`,
        error,
      );
    }
  }
  ...
  private async transformSpuForIndex(spu: any) {
    const minPrice =
      spu.skus.length > 0
        ? Math.min(...spu.skus.map((sku) => Number(sku.originalPrice)))
        : 0;
    const maxPrice =
      spu.skus.length > 0
        ? Math.max(...spu.skus.map((sku) => Number(sku.originalPrice)))
        : 0;
    const minSalePrice =
      spu.skus.length > 0
        ? Math.min(
            ...spu.skus
              .filter((sku) => sku.salePrice)
    ...
    return {
      ...
      // Variations
      variations:
        spu.spuVariations?.map((variation) => ({
      ...
      // Search optimization
      searchKeywords:
        `${spu.name} ${spu.brand?.name} ${spu.category?.name} ${spu.description}`.toLowerCase(),
    };
  }

  private async transformSkuForIndex(sku: any) {
    const finalPrice = sku.salePrice
      ? Number(sku.salePrice)
      : Number(sku.originalPrice);
    const discountPercentage = sku.salePrice
      ? ((Number(sku.originalPrice) - Number(sku.salePrice)) /
          Number(sku.originalPrice)) *
        100
      : 0;
