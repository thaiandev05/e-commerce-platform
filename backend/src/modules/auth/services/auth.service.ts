...
export class AuthService extends OtherService {
  ...
  public async register(dto: RegisterDto) {
    ...
    if (availableAccount)
      throw new BadRequestException('Email or username is available');
    ...
    // send verify code
    await this.emailProducer.sendVerifyCodeRegister({
      to: dto.email,
      code: verifyCode,
    });
  ...
  public async verifyAccountWithCode(
    dto: VerifyAccountWithCodeDto,
  ): Promise<boolean> {
    // validate account
    ...
    // validate account
    if (!existingAccount)
      throw new BadRequestException('Account is not exitsing');
    if (existingAccount.isVerified)
      throw new ForbiddenException('Account already existed');
    ...
    if (!existingCode || !existingCode.expiredAt)
      throw new BadRequestException('Code expired');

    // update new account status
    const userRoleId = 'f142a92f-ff3b-4be1-b65c-da063837ecd5';
  ...
  async oauth2Login(
    user: GoogleOAuth2User | FacebookOAuth2User,
    res: Response,
  ) {
  ...
  public async changePassword(req: Request, dto: ChangePasswordDto) {
    ...
    if (exitstingAccount.hashingPassword) {
      const isMatched = verify(
        exitstingAccount.hashingPassword,
        dto.oldPassword,
      );
  ...
  async softDeleteAccount(req: Request, dto: softDeleteAccountDto) {
    ...
    const isMatched = verify(
      exitsingAccount.hashingPassword || '',
      dto.password,
    );
