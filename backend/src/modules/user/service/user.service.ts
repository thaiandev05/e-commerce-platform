...
export class UserService {
  ...
  async addCreditCard(req: Request, accountId: string, dto: AddCreditCardDto) {
    ...
    if (availableCreditCard)
      throw new ForbiddenException("Credit've used or used by other user");
    ...
    if (isNaN(expired.getTime()))
      throw new BadRequestException(
        'Invalid expiredDate format. Use an ISO date string (e.g. "2030-10-23") or a timestamp',
      );
