...
export class CartService {
  ...
  private async getUserById(userId: string) {
    if (userId === 'unknow')
      throw new BadRequestException('System have not receive userId');
  ...
  private async getProductById(productId: string) {
    if (productId === 'unknow')
      throw new BadRequestException('System have not receive productId');
  ...
  async storeProductToCart(req: Request, productId: string, quantity: number) {
    ...
    if (!availableUser.cart)
      throw new BadRequestException('User have not cart');
  ...
  async deleteProductInCard(
    ...
    const availableStoreProduct =
      await this.prismaService.storeProduct.findUnique({
        where: { id: storeProductId },
      });
    if (!availableStoreProduct)
