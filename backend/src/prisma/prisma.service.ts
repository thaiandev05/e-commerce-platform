  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  async seed() {
    ...
    try {
      ...
      await this.attributeValue.createMany({
        data: seedCollections.attributeValues,
      });
      ...
      await this.storeProduct.createMany({
        data: seedCollections.storeProducts,
      });
      ...
      await this.orderProduct.createMany({
        data: seedCollections.orderProducts,
      });
      ...
      await this.spuVariation.createMany({
        data: seedCollections.spuVariations,
      });
  ...
  private async clearExistingData(): Promise<void> {
    ...
    for (const operation of operations) {
      try {
        ...
        this.logger.warn(
          `Clear operation failed (may be expected):`,
          error.message,
        );
      }
    }
  ...
  private async seedProductRelationships(): Promise<void> {
    ...
    for (const spu of seedCollections.spus.slice(0, 50)) {
      // Only for first 50 SPUs to avoid too much data
      ...
      for (let i = 0; i < numAttributes; i++) {
        const attribute =
          seedCollections.attributes[
            Math.floor(Math.random() * seedCollections.attributes.length)
          ];
        if (usedAttributes.has(attribute.id)) continue;
        usedAttributes.add(attribute.id);

        const availableValues = seedCollections.attributeValues.filter(
          (av) => av.attributeId === attribute.id,
        );
    ...
    for (const spu of seedCollections.spus.slice(0, 80)) {
      // First 80 SPUs get tags
      ...
      for (let i = 0; i < numTags; i++) {
        const tag =
          seedCollections.tags[
            Math.floor(Math.random() * seedCollections.tags.length)
          ];
    ...
    for (const sku of seedCollections.skus.slice(0, 100)) {
      // First 100 SKUs get attributes
      // Each SKU gets 1-2 variation attributes (like color, size)
      const variationAttributes = seedCollections.attributes.filter(
    ...
    if (spuAttributes.length > 0) {
      console.log(
        `🏷️ Creating ${spuAttributes.length} SPU-attribute relationships...`,
      );
    ...
    if (skuAttributes.length > 0) {
      console.log(
        `🔧 Creating ${skuAttributes.length} SKU-attribute relationships...`,
      );
