...
const attributeIds = unique(Array.from({ length: 20 }, () => faker.string.uuid()));
const attributeValueIds = unique(Array.from({ length: 100 }, () => faker.string.uuid()));
...
const categories: Prisma.CategoryCreateManyInput[] = categoryIds.map((id, index) => {
...
const attributes: Prisma.AttributeCreateManyInput[] = attributeIds.map((id, index) => {
...
const attributeValues: Prisma.AttributeValueCreateManyInput[] = attributeValueIds.map((id) => {
...
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
	...
	async seed() {
		...
		try {
			...
			for (const spu of spus) {
				...
				for (let i = 0; i < imageCount; i++) {
					spuImages.push({
						...
						imageUrl: faker.image.urlPicsumPhotos({ width: 800, height: 600 }).slice(0, 500),
			...
			console.log(`📊 Seeded: ${users.length} users, ${categories.length} categories, ${brands.length} brands, ${shops.length} shops, ${spus.length} SPUs, ${skus.length} SKUs`);
